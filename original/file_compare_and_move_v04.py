    ...
    if not possible_duplicate_file == '.DS_Store':
        for possible_original_file in folder_original_files:
            if not possible_original_file == '.DS_store':
                ...
                possible_duplicate_file_pair = os.path.splitext(folder_duplicate + '/' + possible_duplicate_file)
                possible_duplicate_file_name = possible_duplicate_file_pair[0]
                possible_duplicate_file_ext = possible_duplicate_file_pair[1]
                possible_duplicate_file_copy = possible_duplicate_file_name + ' copy' + possible_duplicate_file_ext

                possible_original_file_pair = os.path.splitext(folder_original + '/' + possible_original_file)
                possible_original_file_name = possible_original_file_pair[0]
                possible_original_file_ext = possible_original_file_pair[1]
                possible_original_file_copy = possible_original_file_name + ' copy' + possible_original_file_ext


                if possible_duplicate_file == possible_original_file or possible_duplicate_file_copy == possible_original_file or possible_duplicate_file == possible_original_file_copy:
                    print('Duplicate Detected! Moving ' + possible_duplicate_file + ' because it matches ' + possible_original_file)
